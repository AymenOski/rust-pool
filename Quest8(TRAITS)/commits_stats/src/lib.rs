...
pub fn commits_per_week(data: &JsonValue) -> HashMap<String, u32> {
    ...
    for commit in data.members() {
        if let Some(date_str) = commit["commit"]["author"]["date"].as_str() {
            if let Ok(date) = DateTime::parse_from_rfc3339(date_str) { // Parse the date string to DateTime
                let date_utc: DateTime<Utc> = date.with_timezone(&Utc); // Convert any timezone to UTC
